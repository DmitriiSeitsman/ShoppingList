  ...
  var body: some View {
    if lists.isEmpty {
      ...
      let sorted = lists.sorted { itemA, itemB in
        ...
        case .date: return itemA.createdAt < itemB.createdAt
      ...
      List {
        ForEach(sorted) { item in
          ...
          .swipeActions(edge: .trailing, allowsFullSwipe: true) {
            ...
            Button {
              // позже здесь появится редактирование
            } label: {
      ...
      .alert("Удаление списка", isPresented: $showDeleteConfirm, presenting: pendingDeleteItem) {
        item in
